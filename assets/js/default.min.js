...
$j(document).ready(function () {
    ...
    if ($j('header').hasClass('fixed_hiding')) {
        content_menu_top = min_header_height_fixed_hidden + 20
    }
    ...
    $j([theme_root + '/assets/images/Instate-Site-Logo.png']).preload();
...
function initEdgeSlider() {
    ...
    if ($j('.carousel').length) {
        ...
        $j('.carousel').each(function () {
            ...
            var paspartu_amount_with_top_bottom = $j('.paspartu_outer:not(.disable_top_paspartu):not(.paspartu_on_bottom_slider)').length > 0 ? Math.round($window_width * paspartu_width * 2 + header_height_add_for_paspartu) : 0;
            ...
            if ($this.hasClass('full_screen')) {
                ...
                $j(window).resize(function () {
                    ...
                    paspartu_amount_with_top_bottom = $j('.paspartu_outer:not(.disable_top_paspartu):not(.paspartu_on_bottom_slider)').length > 0 ? Math.round($window_width * paspartu_width * 2 + header_height_add_for_paspartu) : 0;
                ...
                $this.find('.edgt_slider_preloader').css({
                    'height': ($this.height() - mobile_header - paspartu_amount_with_top_bottom - paspartu_amount_with_bottom) + 'px',
                    'display': 'block'
                });
                if ($j('.paspartu_outer:not(.disable_top_paspartu):not(.paspartu_on_bottom_slider)').length) {
                    if (!$j('body').hasClass('paspartu_on_top_fixed')) {
            ...
            if ($j('body:not(.boxed):not(.vertical_menu_transparency):not(.vertical_menu_background_opacity):not(.vertical_menu_hidden):not(.page-template-landing_page-php)').hasClass('vertical_menu_enabled') && $window_width > 1000) {
                ...
                if ($j('.vertical_menu_width_350').length) {
                    var vertical_menu_width = 350
                }
                if ($j('.vertical_menu_width_400').length) {
                    var vertical_menu_width = 400
                }
            ...
            function initSlider() {
                ...
                if ($this.hasClass('edgt_auto_start')) {
                    ...
                    $this.find('.slide_buttons_holder .qbutton').mouseenter(function () {
                        $this.carousel('pause')
                    }).mouseleave(function () {
                        $this.carousel('cycle')
                    })
                } else {
                    $this.carousel({interval: 0, pause: false})
                }
                ...
                if ($j('.carousel-inner .item:first-child').hasClass('animate_image') && $window_width > 1000) {
                    $this.find('.carousel-inner .item.animate_image:first-child .image').transformAnimate({
                        ...
                        duration: 30000,
                        complete: function () {
                            $j(this).transformAnimate({transform: "matrix(1, 0, 0, 1, 0, 0)", duration: 30000})
                        }
                    })
                }
            }
...
function setDropDownMenuPosition() {
    ...
    menu_items.each(function (i) {
        ...
        if ($j('body').hasClass('boxed')) {
            menu_item_from_left = boxed_layout - (menu_item_position - (browser_width - boxed_layout) / 2) + 24
        } else {
            menu_item_from_left = browser_width - menu_item_position + 24
        }
    ...
    if ($j('.no-touch .vertical_menu').hasClass('click')) {
        ...
        menu_items.each(function (i) {
            if ($j(menu_items[i]).parent().hasClass('has_sub')) {
                ...
                var link = $j("<a />", {
                    "href": menu_url,
                    "html": '<i class="edgt_menu_arrow fa fa-angle-left"></i>' + menu_text
                }).appendTo(li);
        ...
        $j('.counter.zero').each(function () {
            if (!$j(this).hasClass('executed')) {
                ...
                $j(this).appear(function () {
                    ...
                    $j(this).countTo({from: 0, to: $max, speed: 1500, refreshInterval: 100})
...
function initToCounterHorizontalProgressBar($this) {
    ...
    if ($this.find('.progress_number span.percent').length) {
        $this.find('.progress_number span.percent').each(function () {
            ...
            $j(this).countTo({from: 0, to: percentage, speed: 1500, refreshInterval: 50})
        })
    }
}
        ...
        $j('.animate_list').each(function () {
            $j(this).appear(function () {
                $j(this).find("li").each(function (l) {
                    var k = $j(this);
                    setTimeout(function () {
                        k.animate({opacity: 1, top: 0}, 1500)
                    }, 100 * l)
                })
            }, {accX: 0, accY: element_appear_amount})
        })
...
function initToCounterPieChart($this) {
    ...
    $j($this).find('.tocounter').countTo({from: 0, to: $max, speed: 1500, refreshInterval: 50})
}
    ...
    if ($j('.projects_holder_outer:not(.masonry_with_space)').length) {
        $j('.projects_holder_outer').each(function () {
            ...
            currentPortfolio.mixitup({
                showOnLoad: 'all', transitionSpeed: 600, minHeight: 150, onMixLoad: function () {
        ...
        $j('.projects_masonry_holder, .masonry_with_space .projects_holder').each(function () {
            ...
            $this.waitForImages(function () {
                ...
                $this.isotope({
                    ...
                    masonry: {columnWidth: '.portfolio_masonry_grid_sizer'}
                })
            })
        });
    ...
    if ($j('.parallax_section_holder').length) {
        $j('.parallax_section_holder').each(function () {
            if ($j(this).hasClass('full_screen_height_parallax')) {
                $j(this).height($window_height);
                $j(this).find('.parallax_content_outer').css('padding', 0)
            }
            ...
            if ($window.width() > 960) {
                if ($window.scrollTop() + $headerHeight > offset.top) {
                    if ($window.scrollTop() + $headerHeight + $sidebar.height() + 24 < $scrollOffset.top + $scrollHeight) {
                        $sidebar.stop().animate({marginTop: $window.scrollTop() - offset.top + $headerHeight})
                    } else {
                        $sidebar.stop().animate({marginTop: $scrollHeight - $sidebar.height() - 24})
                    }
                } else {
        ...
        $j('.edgt_progress_bars_icons_holder').each(function () {
            ...
            $this.appear(function () {
                ...
                $this.find('.edgt_progress_bars_icons').each(function () {
                    ...
                    if (size !== "") {
                        $j(this).find('.edgt_progress_bars_icons_inner.custom_size .bar').css({
                            'width': size + 'px',
                            'height': size + 'px'
                        });
            ...
            $this.find('.more_facts_button').click(function () {
                if (!$this.find('.more_facts_inner_holder').is(':visible')) {
                    ...
                    $j(this).parent().parent().find('.more_facts_inner_holder').css({
                        'display': 'block',
                        'opacity': '1'
                    }).stop().animate({'height': height + 30}, speed, function () {
                        if ($j('.parallax_section_holder').length) {
                            initParallax()
                        }
                    });
    ...
    if ($j('.edgt_progress_bars_vertical').length) {
        $j('.edgt_progress_bars_vertical').each(function () {
            $j(this).appear(function () {
                ...
                $j(this).find('.progress_content').animate({height: percentage + '%'}, 1500)
            }, {accX: 0, accY: element_appear_amount})
        })
    }
}
...
function initToCounterVerticalProgressBar($this) {
    ...
    if ($this.find('.progress_number span').length) {
        $this.find('.progress_number span').each(function () {
            var $max = parseFloat($j(this).text());
            $j(this).countTo({from: 0, to: $max, speed: 1500, refreshInterval: 50})
        })
    }
}
        ...
        if ($j('header.page_header').hasClass('fixed') && !$j('body').hasClass('vertical_menu_enabled')) {
            ...
            if (!$j('header.page_header').hasClass('transparent') || $j('header.page_header').hasClass('scrolled_not_transparent')) {
                if ($j('[data-edgt_id="' + hash + '"]').offset().top - (header_height + logo_height / 2 + 20) <= scroll_amount_for_fixed_hiding) {
                    scrollToAmount = $j('[data-edgt_id="' + hash + '"]').offset().top - header_height - logo_height / 2 - 20
                } else {
                    scrollToAmount = $j('[data-edgt_id="' + hash + '"]').offset().top - min_header_height_fixed_hidden - 20
                }
            } else {
                scrollToAmount = $j('[data-edgt_id="' + hash + '"]').offset().top
            }
            ...
            if (!$j('header.page_header').hasClass('transparent') || $j('header.page_header').hasClass('scrolled_not_transparent')) {
                if (sticky_amount >= $j('[data-edgt_id="' + hash + '"]').offset().top) {
                    scrollToAmount = $j('[data-edgt_id="' + hash + '"]').offset().top + 1
                } else {
...
function initHashClick() {
    ...
    $j(document).on("click", ".main_menu a, .vertical_menu a, .popup_menu a, .qbutton, .anchor, .widget li.anchor a", function () {
        ...
        if (loadedPageFlag == true && ((hash !== "" && $j(this).attr('href').split('#')[0] === "") || (hash !== "" && $j(this).attr('href').split('#')[0] !== "" && hash === window.location.hash) || (hash !== "" && $j(this).attr('href').split('#')[0] == window.location.href.split('#')[0]))) {
            ...
            if ($j('[data-edgt_id="' + hash + '"]').length > 0) {
                $doc.stop().animate({scrollTop: Math.round(scrollToAmount)}, 1500, function () {
    ...
    $j(document).on("click", ".mobile_menu a", function () {
        ...
        if ((hash !== "" && $j(this).attr('href').split('#')[0] === "") || (hash !== "" && $j(this).attr('href').split('#')[0] !== "" && hash === window.location.hash) || (hash !== "" && $j(this).attr('href').split('#')[0] == window.location.href.split('#')[0])) {
            if ($j('[data-edgt_id="' + hash + '"]').length > 0) {
                $doc.animate({scrollTop: Math.round($j('[data-edgt_id="' + hash + '"]').offset().top - $j('.mobile_menu').height())}, 500, function () {
                    anchorActiveState($this)
                })
            }
            if (history.pushState) {
...
function initializeGoogleMap(custom_map_style, color, saturation, lightness, wheel, zoom, holderId, height, pin, map, geocoder, data, show_address_box) {
    ...
    var edgtMapType = new google.maps.StyledMapType(mapStyles, {name: "Edge Google Map"});
...
function contentMenuScrollTo() {
    ...
    $j("nav.content_menu ul.menu li a").on('click', function (e) {
        ...
        if ($j('header.page_header').hasClass('fixed') && !$j('body').hasClass('vertical_menu_enabled')) {
            ...
            if (!$j('header.page_header').hasClass('transparent')) {
                if ($j('[data-edgt_id="' + $target + '"]').offset().top - (header_height + content_menu_top_add + logo_height / 2 + 20) <= scroll_amount_for_fixed_hiding) {
                    scrollToAmount = $j('[data-edgt_id="' + $target + '"]').offset().top - content_menu_top_add - header_height - logo_height / 2 - 20
                } else {
                    scrollToAmount = $j('[data-edgt_id="' + $target + '"]').offset().top - content_menu_top_add - min_header_height_fixed_hidden - 20
                }
            } else {
                scrollToAmount = $j('[data-edgt_id="' + $target + '"]').offset().top
            }
            ...
            if (!$j('header.page_header').hasClass('transparent')) {
                if (sticky_amount >= $j('[data-edgt_id="' + $target + '"]').offset().top) {
                    scrollToAmount = $j('[data-edgt_id="' + $target + '"]').offset().top + 1
                } else {
        ...
        if ($j('[data-edgt_id="' + $target + '"]').length > 0) {
            $doc.stop().animate({scrollTop: Math.round(scrollToAmount - $j('nav.content_menu').height())}, 1500, function () {
                $j('nav.content_menu ul li').removeClass('active');
...
function initSocialIconHover() {
    ...
    if ($j('.edgt_social_icon_holder').length) {
        $j('.edgt_social_icon_holder').each(function () {
            ...
            if (hover_style !== "") {
                if ($j(this).find('.edgt_icon_stack .social_icon').length) {
                    $j(this).find('.edgt_icon_stack').hover(function () {
    ...
    if ($j('a.popup_menu').length) {
        ...
        if ($j('body').hasClass('fade_push_text_right') || $j('body').hasClass('fade_push_text_top')) {
            if ($j('.fullscreen_above_menu_widget_holder > div').length) {
                ...
                $j('.popup_menu_holder_outer nav > ul > li > a').each(function (i) {
                    $j(this).css({'-webkit-animation-delay': i * 70 + 'ms', 'animation-delay': i * 70 + 'ms'})
                });
                $j('.fullscreen_menu_widget_holder > div').css({
                    '-webkit-animation-delay': ($j('.popup_menu_holder_outer nav > ul > li > a').length + 1) * 70 + 'ms',
                    'animation-delay': ($j('.popup_menu_holder_outer nav > ul > li > a').length + 1) * 70 + 'ms'
        ...
        $j('.edgt_image_gallery_no_space').each(function () {
            if ($j(this).hasClass('full_screen_height')) {
                $j(this).find('ul li img').each(function () {
                    ...
                    var image_width = Math.ceil($j(this).width() * proportion) - 1;
...
function initVerticalSplitSlider() {
    ...
    if ($j('.vertical_split_slider').length) {
        ...
        $j('.vertical_split_slider').multiscroll({
            scrollingSpeed: 500, navigation: true, useAnchorsOnLoad: false, afterRender: function () {
...
function drawSVG(svg) {
    "use strict";
    var svgs = Array.prototype.slice.call(svg.find('svg')), svgArr = new Array(), resizeTimeout;
...
function initBlogMasonryGallery() {
    ...
    if ($j('.blog_holder.blog_masonry_gallery').length) {
        $j('.blog_holder.blog_masonry_gallery').each(function () {
            ...
            $this.waitForImages(function () {
                $this.animate({opacity: 1});
                $this.isotope({itemSelector: 'article', masonry: {columnWidth: '.blog_holder_grid_sizer'}})
            })
        });
        $j(window).resize(function () {
            resizeBlogMasonryGallery($j('.blog_holder_grid_sizer').width());
